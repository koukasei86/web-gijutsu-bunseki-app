...
export function buildPrototype(profile: SiteProfile, kind: SiteKind, brief: string, options: PrototypeOptions = {}) {
  ...
  const names = profile.headings.slice(1, 4);
  const cards = labels.map((label, i) => `<article class="card" data-category="${i % 2 ? 'おすすめ' : '新着'}"><div class="art art-${i}" aria-hidden="true">${['01', '02', '03'][i]}</div><div class="card-body"><small>${i % 2 ? 'おすすめ' : '新着'} / SAMPLE</small><h2>${escapeHtml(names[i] || label)}</h2><p>学習用のサンプルです。実際の商品・作品・記事ではありません。</p>${kind === 'shop' ? `<p>参考価格 ¥${[2400, 1800, 900][i].toLocaleString()}</p>` : ''}<button class="detail" type="button">${kind === 'shop' ? 'カートに追加' : '詳細を見る'}</button></div></article>`).join('\n');
  const note = brief.trim() || profile.description || descriptions[kind];
